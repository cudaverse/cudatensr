...
tarball with the current R release, records its SHA-256, then checks that exact
tarball with R-devel, including the reference manual. The candidate and its
check evidence are retained together. CRAN acceptance is never inferred from
an ordinary GitHub check.
